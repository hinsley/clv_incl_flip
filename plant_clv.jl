...
gh = Float64(Plant.default_params[4])
ΔCa = 15.0
Δx  = -2.4
...
origins = [Point3f(u[5], u[1], u[6]) for u in xs[1:stride:end]]
...
real_j = [1, 0, 2, 3, 4, 5] # Hack: Since we don't use y it is skipped.
for j in 1:nclv
    origins_j = Point3f[]
    dirs_j = Vec3f[]
    for (k, i) in enumerate(1:stride:length(Gamma))
        vfull = Gamma[i][:, j]
